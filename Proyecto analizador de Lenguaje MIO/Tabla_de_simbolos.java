...
public class Tabla_de_simbolos {
    ...
    public String verificarExistencia(String cadena) {
        for (int i = 0; i < 11; i++) {
            if ((i < PalabrasReservadas.length && cadena.equals(PalabrasReservadas[i])) ||
                ...
                (i < Identificadores.length && Identificadores[i][1] != null && cadena.equals(Identificadores[i][1])) ||
                (i < LiteralesTxt.length && LiteralesTxt[i][1] != null && cadena.equals(LiteralesTxt[i][1])) ||
                (i < LiteralesNum.length && LiteralesNum[i][1] != null && cadena.equals(LiteralesNum[i][1]))) {
                return cadena;
            }
        }
        return null;
    ...
    public String verificarIdentificador(String cadena) {
        if (cadena.length() > 0 && cadena.charAt(0) != '0' && cadena.charAt(0) != '1' && cadena.charAt(0) != '2' &&
            ...
            if (cadena.length() <= 16) {
                int i = 0;
                while (i < 2000 && Identificadores[i][1] != null) {
                    i++;
                }
                if (i < 2000) {
    ...
    public String verificarLiteralesTxt(String cadena) {
        if (cadena.length() > 0 && cadena.charAt(0) == '"') {
            if (cadena.length() >= 2 && cadena.charAt(cadena.length() - 1) == '"') {
                int i = 0;
                while (i < 2000 && LiteralesTxt[i][1] != null) {
                    i++;
                }
                if (i < 2000) {
                    ...
                    System.out.println("Error: No hay espacio para más literales de texto");
                }
            } else {
    ...
    public String verificarLiteralesNum(String cadena) {
        if (cadena.startsWith("0X") || cadena.startsWith("0x")) {
            int i = 0;
            while (i < 2000 && LiteralesNum[i][1] != null) {
                i++;
            }
            if (i < 2000) {
                ...
                LiteralesNum[i][0] = "NUM" + i;
                LiteralesNum[i][1] = cad;
                return LiteralesNum[i][0];
            } else {
                System.out.println("Error: No hay espacio para más literales numéricos");
            }
        }
        return null;
    }
